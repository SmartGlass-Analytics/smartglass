...
API_FILE= join(dirname(__file__), "alabama_api.txt")
...
BOX_JSON_FILE = join(os.environ["HOME"], "BoxScore.json")
BOX_TEXT_FILE = join(os.environ["HOME"], "BoxScore.txt")
...
def printPlayerStats(boxJS):
    with open(BOX_TEXT_FILE, "w") as file:
...
lineupStats("Mark Sears", "Aden Holloway",  "Mouhamed Dioubate", "Grant Nelson", "Aiden Sherrell")